...
module.exports = class Audits {
    ...
    static sourceAlbumTagsInconsistent(metadata /*, contentToSync */) {
        console.log('sourceAlbumTagsInconsistent');
        ...
        Object
            ...
            .map(entry => {
                ...
                const audioFilePaths = entry[1];

                audioFilePaths.map(item => {
                    const audioFilePath = item.audioFilePath;
                    const trackMetadata = metadata.audioFilePathToMetadata[audioFilePath];

                    if (trackMetadata.common.genre !== undefined && trackMetadata.common.genre.length > 0) {
                        genres.add(trackMetadata.common.genre[0]);
                    }

                    albumArtists.add(trackMetadata.common.albumartist);
                    dates.add(trackMetadata.common.date);
                });

                if (albumArtists.size !== 1 || genres.size !== 1 || dates.size !== 1) {
                    let line = `Album "${albumName}":\r\n`;

                    if (albumArtists.size !== 1) {
                        line += `\t\ttracks do not have exactly one Album Artist\r\n`;
                    }

                    if (genres.size !== 1) {
                        line += `\t\ttracks do not have exactly one Genre\r\n`;
                    }

                    if (dates.size !== 1) {
                        line += `\t\ttracks do not have exactly one Date\r\n`;
                    }
        ...
        return {
            header: 'Source Album Tags Are Inconsistent',
            lines
        };
    }

    // Sometimes the name of the album is the prefix of every track title. The user may want to remove this from each track.
    static sourceAlbumTracksHaveCommonPrefix(metadata /*, contentToSync */) {
        console.log('sourceAlbumTracksHaveCommonPrefix');
            ...
            .map(entry => {
                ...
                audioFilePaths.map(item => {
                    let element = {
                        'metadata': {
                            'common': {
                                'title': metadata.audioFilePathToMetadata[item.audioFilePath]['common']['title']
                            }
                        }
                    };

                    trackMetadata.push(element);
                });

                let prefix = TrackUtils.prefix(trackMetadata);
        ...
        return {
            header: 'Source Album Tracks Have Common Prefix',
            lines: lines.sort()
        };
    }
    ...
    static targetAlbumsHaveDiskOrTrackGapsOrNonUniqueTrackNumbers(metadata /*, contentToSync */) {
        console.log('targetAlbumsHaveDiskOrTrackGapsOrNonUniqueTrackNumbers');
        ...
        Object
            ...
            .map(entry => {
                ...
                tracks.map(track => {
                    ...
                    const diskNumber = trackMetadata.common?.disk?.no;
                    let discData = albumDiscsAndTracks[diskNumber];

                    if (discData === undefined) {
                        discData = {};

                        albumDiscsAndTracks[diskNumber] = discData;
                    }

                    const trackNumber = trackMetadata.common?.track?.no;

                    if (discData[trackNumber] === undefined) {
                        discData[trackNumber] = true;
                    } else {
                        lines.push(`Album "${album}": There are multiple tracks with a Track # of ${trackNumber}\r\n`);
                    }
                })
        ...
        return {
            header: 'Target Albums Have Disk Or Track Gaps, or Non-Unique Track Numbers',
            lines
        };
    }
