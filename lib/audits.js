...
module.exports = class Audits {
    ...
    static albumNamesDoNotMatchPlaylists(metadata /*, contentToSync */) {
        console.log(`albumNamesDoNotMatchPlaylists`);
        ...
        Object
            .entries(metadata.playlists)
            .map(entry => {
                const playlistName = path.basename(entry[0], '.m3u');
                playlistFilenames.add(playlistName);
        });
        ...
        Object
            ...
            .map(entry => {
                const albumName = entry[0].replace(/ \(Disc [0-9]+ of [0-9]+\)/g, '');

                if (!processedAlbumNames.has(albumName) && !playlistFilenames.has(albumName)) {
                    lines.push(`"${albumName}"\r\n`);

                    processedAlbumNames.add(albumName);
                }
            });
        ...
        return {
            header: 'Album Names Do Not Match Playlists',
            lines
        };
    }
