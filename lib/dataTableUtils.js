...
module.exports = class DataTableUtils {
    static toTableData(metadata, selectedItemType) {
        let result = undefined;

        switch(selectedItemType) {
            case StringLiterals.ITEM_TYPE_PLAYLISTS: {
                result = this._getItems(metadata.playlists, selectedItemType);
            }
            break;

            case StringLiterals.ITEM_TYPE_ALBUMS: {
                result = this._getItems(metadata.albums, selectedItemType);
            }
            break;
    ...
    static trackArrayToTableData(trackArray) {
        ...
        return trackArray.map(track => ({
                ...
                'playlistPosition': `${formatData(playlistPosition++)}`
            }));
    }
    ...
    static _getItems(items, type) {
        ...
        Object.entries(items).forEach(function([name]) {
    ...
    static getTracksTableColumns(createContextMenu, deleteTrack, cellEdited, printIconDelete) {
        return [
            ...
            {title: "Title", field: "title", width: 200, responsive: 0,
                editor: StringLiterals.GRID_EDITOR_INPUT, cellEdited: cellEdited},
            {formatter:printIconDelete, headerSort: false, width:30, cellClick:function(e, cell) { deleteTrack(e, cell); }},
            ...
            {title: "Track #", field: "trackNumber", width:100, responsive: 0,
                editor: StringLiterals.GRID_EDITOR_NUMBER, cellEdited: cellEdited,
                editorParams: Constants.SEQUENCE_EDITOR_PARAMS, sorter: StringLiterals.GRID_SORTER_NUMBER},
            {title: "Playlist Position", field: StringLiterals.COLUMN_PLAYLIST_POSITION, width: 100, responsive: 0,
