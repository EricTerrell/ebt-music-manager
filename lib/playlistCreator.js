...
module.exports = class PlaylistCreator {
    static write(playlistFilePath, tracks) {
        ...
        const fileName = FileSystemUtils.validFileName(path.basename(playlistFilePath));

        playlistFilePath = path.join(folderPath, fileName);
    ...
    static getFullPath(playlistFilename) {
        ...
        return path.join(settings.sourceFolder, StringLiterals.PLAYLISTS_TARGET_FOLDER, `${playlistFilename}.${StringLiterals.PLAYLIST_FILE_TYPE}`);
    }
};
