...
module.exports = class AudioFileUtils {
    static isFlac(filePath) {
        return AudioFileUtils.#getFileType(filePath, true) === `.${StringLiterals.FLAC_FILE_TYPE}`;
    }

    static isMP3(filePath) {
        return AudioFileUtils.#getFileType(filePath, true) === `.${StringLiterals.MP3_FILE_TYPE}`;
    }

    static isPlaylist(filePath) {
        return AudioFileUtils.#getFileType(filePath, true) === `.${StringLiterals.PLAYLIST_FILE_TYPE}`;
    }
    ...
    static getTargetFilePath(metadata, settings, sourceFilePath, targetFolderPath) {
        ...
        if (AudioFileUtils.isPlaylist(sourceFilePath)) {
            ...
            const album = Hash.generateHash(metadata.audioFilePathToMetadata[sourceFilePath].common.album);
            const fileType = path.extname(sourceFilePath);
            const fileName = Hash.generateHash(path.basename(sourceFilePath, fileType));
