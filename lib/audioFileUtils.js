...
module.exports = class AudioFileUtils {
    ...
    static getTargetFilePath(metadata, settings, sourceFilePath, targetFolderPath) {
        ...
        if (AudioFileUtils.isPlaylist(sourceFilePath)) {
            ...
            try {
                album = Hash.generateHash(fileMetadata.common.album);

                disc = NumericUtils.zeroPad(fileMetadata.common.disk.no, 3);
                track = NumericUtils.zeroPad(fileMetadata.common.track.no, 3);
            } catch (ex) {
                const errorMessage = `Audio file "${sourceFilePath}" lacks either an album, disk number, or track number.`;
