...
module.exports = class AudioFileUtils {
    ...
    static shouldTranscode(settings, filePath) {
        ...
        return settings.audioFileTypeActions.find(item => item.fileType === fileType && item.action === StringLiterals.CONVERT_TO_MP3) !== undefined;
    }

    static shouldCopyAsIs(settings, filePath) {
        const fileType = path.extname(filePath).slice(1).toLowerCase();

        return settings.audioFileTypeActions.find(item => item.fileType === fileType && item.action === StringLiterals.COPY) !== undefined;
    }
