...
module.exports = class Filter {
    ...
    static _getValues(fieldName, filterData) {
        const fieldNameToFunction = {
            ...
            albumArtist: filterData => filterData.tracks.map(element => this._formatData(element.metadata?.common?.albumartist)),
            artist: filterData => filterData.tracks.map(element => this._formatData(element.metadata?.common?.artist)),
            composer: filterData => filterData.tracks.map(element => this._formatData(element.metadata?.common?.composer)),
    ...
    static _getTracks(trackPaths, metadata) {
        const result = Object.values(trackPaths)
            .map(value => {
